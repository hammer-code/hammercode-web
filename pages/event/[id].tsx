...
function TimeIcon() {
  return (
    <svg xmlns="http://www.w3.org/2000/svg" width="24" height="24" viewBox="0 0 24 24" fill="none" className="injected-svg text-gray5 fill-current" data-src="https://secure.meetupstatic.com/next/images/design-system-icons/time-outline.svg"><title>icon</title>
...
function MapMarkerIcon() {
  return (
    <svg xmlns="http://www.w3.org/2000/svg" width="24" height="24" viewBox="0 0 24 24" fill="none" className="injected-svg text-gray5 fill-current" data-src="https://secure.meetupstatic.com/next/images/design-system-icons/map-marker-outline.svg"><title>icon</title>
    ...
    <MainLayout>
      ...
      <div className="mx-auto container">
        <div className="py-8">
          ...
          <h1 className="font-bold text-3xl mb-8 mt-2 text-gray-700">{event.name}</h1>
        </div>
      </div>
            ...
            <div className="col-span-12 md:col-start-9 md:col-end-12">
              <div className="bg-white p-6 rounded-t-lg">
                <div className="mb-4 flex">
                  <span className="text-gray-400"><TimeIcon /></span> <span className="ml-3">{formatDate(event.date)}</span>
                </div>
                <div className="flex">
                  <span className="text-gray-400"><MapMarkerIcon /></span>
                  <div className="ml-3">
                    <p>Hotel Santika</p>
                    <p className="text-gray-500">Jl. Moh. Hatta No. 18, Lolu Utara</p>
                  </div>
                </div>
              </div>
              <div>
        ...
        <div className="mx-auto container">
          <div className="flex">
            ...
            <div className="flex">
              ...
              <button className="py-4 px-6 bg-black text-white rounded">Ikut Acara</button>
            </div>
          </div>
        </div>
      </div>
...
export async function getServerSideProps(ctx: GetServerSidePropsContext & { locale: string }) {
  ...
  try {
    event = await fetch(process.env.NEXT_PUBLIC_API_BASE_URL + '/api/v1/events/' + id)
      .then(res => res.json())
      .then(res => {
        return res.data;
      });
