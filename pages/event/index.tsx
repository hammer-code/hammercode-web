...
function EventItem ({ locale, event }: { locale: string, event: HMCEvent }) {
  return (
      <div data-id={event.id} className="p-4 border-b">
        <div className="flex">
          <div style={{ width: 160, height: 86 }} className="mr-4 bg-gray-200 rounded"></div>
          <div>
            <p className="text-yellow-700">{formatDate(event.date, locale)}</p>
            <h2 className="font-bold text-xl">
              <Link href={`/event/${event.id}`}><a>{event.name}</a></Link>
            </h2>
...
export default function EventListPage({ locale, events }: { locale: string, events: HMCEvent[] }) {
  return (
    <MainLayout>
...
export async function getServerSideProps(ctx: GetServerSidePropsContext & { locale: string }) {
  let events: HMCEvent[] = [];
  try {
    events = await fetch(process.env.NEXT_PUBLIC_API_BASE_URL + '/api/v1/events')
      .then(res => res.json())
      .then(res => {
        return res.data;
      });
